...
<head>
    ...
    <style>
        ...
        .ts-export-menu {
            ...
            border: 1px solid rgba(74,108,247,0.22); border-radius: var(--radius);
        ...
        .ts-export-menu a {
            ...
            font-family: 'Times New Roman', Times, serif; font-size: 13.5px; font-style: italic;
            transition: var(--transition); border-bottom: 1px solid var(--border-light);
        }
        .ts-export-menu a:last-child { border-bottom: none; }
        .ts-export-menu a:hover { background: rgba(74,108,247,0.10); color: var(--blue-light); }
        ...
        .ts-kpi-chip {
            ...
            font-family: 'Times New Roman', Times, serif; font-size: 11px; font-style: italic; margin-top: 5px;
        }
        .ts-kpi-chip.up   { background: rgba(74,222,128,0.12); color: #4ade80; border: 1px solid rgba(74,222,128,0.2); }
        .ts-kpi-chip.down { background: rgba(232,68,90,0.12);  color: #f87171; border: 1px solid rgba(232,68,90,0.2); }
        ...
        .ts-prod-bar       { height: 6px; border-radius: 3px; background: linear-gradient(90deg,#e8445a,#4a6cf7); min-width: 4px; }
                ...
                <div class="ts-card-header">
                    ...
                    <div style="display:flex;gap:14px;align-items:center">
                        <span style="font-family:'Times New Roman',Times,serif;font-size:11px;font-style:italic;color:var(--text-muted)">
                            <span style="display:inline-block;width:10px;height:3px;border-radius:2px;background:#e8445a;margin-right:5px;vertical-align:middle"></span>Ventes
                        </span>
                        <span style="font-family:'Times New Roman',Times,serif;font-size:11px;font-style:italic;color:var(--text-muted)">
                            <span style="display:inline-block;width:10px;height:3px;border-radius:2px;background:#4a6cf7;margin-right:5px;vertical-align:middle"></span>Commandes
                        </span>
                    </div>
                </div>
        ...
        <div class="ts-card">
            <div class="ts-card-header">
                <div class="ts-card-title">
                    <i class="fas fa-trophy" style="background:linear-gradient(135deg,#fbbf24,#e8445a);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text"></i>
...
const R='#e8445a',PK='#d4527c',BL='#4a6cf7',BLI='#6b8cff',PU='#7c4dff',GR='#4ade80',GO='#fbbf24',TX='rgba(200,215,255,0.55)',GD='rgba(100,140,255,0.08)';
Chart.defaults.color=TX; Chart.defaults.borderColor=GD; Chart.defaults.font.family="'Times New Roman',Times,serif";
...
},{options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{display:false},tooltip:{backgroundColor:'rgba(8,12,28,0.96)',borderColor:'rgba(74,108,247,0.22)',borderWidth:1,padding:12}},scales:{x:{grid:{display:false},ticks:{font:{size:11,style:'italic'}}},y:{position:'left',grid:{color:GD},beginAtZero:true},y1:{position:'right',grid:{drawOnChartArea:false},beginAtZero:true}}}});
...
},{options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{padding:14,font:{size:11,style:'italic'},color:TX,usePointStyle:true,pointStyleWidth:10}},tooltip:{backgroundColor:'rgba(8,12,28,0.96)',borderColor:'rgba(74,108,247,0.22)',borderWidth:1,padding:10}},cutout:'68%'}});
...
if(topCtx){topChart=new Chart(topCtx,{type:'bar',data:{labels:topLabels,datasets:[{label:'Unités vendues',data:topQtys,backgroundColor:[R,PK,BL,PU,BLI,GO,GR],borderRadius:7,borderSkipped:false}]},{options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{backgroundColor:'rgba(8,12,28,0.96)',borderColor:'rgba(74,108,247,0.22)',borderWidth:1,padding:10}},scales:{x:{grid:{color:GD},beginAtZero:true},y:{grid:{display:false},ticks:{font:{size:11,style:'italic'}}}}}}});}

const dailyCtx=document.getElementById('dailySalesChart')?.getContext('2d');
if(dailyCtx){const days=['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'];new Chart(dailyCtx,{type:'bar',data:{labels:days,datasets:[{label:'Ventes',data:<?= json_encode($dailyData??[0,0,0,0,0,0,0]) ?>,backgroundColor:days.map(function(_,i){const g=dailyCtx.createLinearGradient(0,0,0,200);g.addColorStop(0,i<5?R:BL);g.addColorStop(1,i<5?PK+'88':PU+'88');return g;}),borderRadius:8,borderSkipped:false,borderWidth:0}]},{options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{backgroundColor:'rgba(8,12,28,0.96)',borderColor:'rgba(74,108,247,0.22)',borderWidth:1,padding:10}},scales:{x:{grid:{display:false},ticks:{font:{size:10,style:'italic'}}},y:{grid:{color:GD},beginAtZero:true}}}}});}

const stCtx=document.getElementById('statusChart')?.getContext('2d');
if(stCtx) new Chart(stCtx,{type:'bar',data:{labels:['En attente','Confirmé','Préparation','Expédié','Livré','Annulé'],datasets:[{label:'Commandes',data:<?= json_encode([$statusCounts['en_attente']??0,$statusCounts['confirme']??0,$statusCounts['en_preparation']??0,$statusCounts['expedie']??0,$statusCounts['livre']??0,$statusCounts['annule']??0]) ?>,backgroundColor:[GO,BLI,R,PU,GR,'#f87171'],borderRadius:8,borderSkipped:false}]},{options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{backgroundColor:'rgba(8,12,28,0.96)',borderColor:'rgba(74,108,247,0.22)',borderWidth:1,padding:10}},scales:{x:{grid:{display:false},ticks:{font:{size:10,style:'italic'}}},y:{grid:{color:GD},beginAtZero:true}}}});
